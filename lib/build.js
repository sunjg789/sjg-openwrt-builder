    ...
    try {
      ...
      if (st.status === 'completed') {
        ...
        if (arts.length && !pulling.has(id)) {
          pulling.add(id);
          try {
            // 上一轮已经拉成功的不再重下：369MB 的产物重复下载代价太大
            ...
            for (const a of arts) {
              ...
              try {
                ...
                got.push({ name: `${a.name}.zip`, file, size: out.size, origin: a.name });
              } catch (e) {
                got.push({ name: `${a.name}.zip`, file, error: e.message, origin: a.name });
              }
            }
            meta.files = got;
            meta.downloadAttempts = (meta.downloadAttempts || 0) + 1;
            meta.downloaded = got.every((f) => !f.error);
            writeMeta(id, meta);
          } finally {
            pulling.delete(id);
          }
        ...
        // 下载抖动不能把这次永久判死：只要还有没拉下来的，就留着下次轮询重试
        const stillBad = (meta.files || []).filter((f) => f.error);
        if (!arts.length || meta.downloaded) {
          meta.terminal = true;
        } else if (meta.downloadAttempts >= 3) {
          meta.terminal = true;
          meta.error = `产物拉取失败（已重试 ${meta.downloadAttempts} 次）：${stillBad.map((f) => f.error).join('；')}`;
        }
        writeMeta(id, meta);
      }
    } catch (e) {
