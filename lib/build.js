...
async function startBuild(spec, files, { token: tokenIn, repo: repoIn } = {}) {
  ...
  try {
    ...
    // 即使内容正确，dispatch 也会被拒（404，或列表有幽灵记录时的 422）。
    // 判定必须看默认分支上有没有这个文件（isRegisteredOnDefault 内部走 contents 接口），
    // 不能信 /actions/workflows 列表——它会残留已删除文件的记录。
    let registered = await gh.isRegisteredOnDefault(workflowFile).catch(() => null);

    // 不在默认分支就自己补上（只补这一个 yml，构建包不会混进默认分支）
    if (!registered) {
      const wf = files.find((f) => f.path === `.github/workflows/${workflowFile}`);
      if (wf) {
        try {
          const put = await gh.putFileOnDefault(wf, `chore: 注册 ${workflowFile}（workflow_dispatch 只认默认分支上的工作流）`);
          meta.workflowPushedTo = put.branch;
          meta.workflowPushedAt = new Date().toISOString();
          registered = await gh.isRegisteredOnDefault(workflowFile).catch(() => null);
        } catch (e) {
          meta.workflowPushError = e.message;
        }
      }
    }
    ...
    if (registered) {
      try {
        ...
        // dispatch 被拒（422 幽灵注册 / 404 未注册）时不要把任务判死：
        // 去掉 [skip ci] 重推一次，让 push 触发器接管。
        meta.dispatchError = e.message;
        meta.trigger = 'push_fallback';
        meta.registeredOnDefault = false;
        writeMeta(id, meta);
