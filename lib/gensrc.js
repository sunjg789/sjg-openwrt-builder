      ...
      - name: 依赖
        run: |
          ...
          sudo apt-get install -y ack antlr3 asciidoc autoconf automake autopoint binutils bison build-essential \\
            ...
            libtool libz-dev lrzsz mkisofs msmtp nano ninja-build p7zip p7zip-full patch pkgconf \\
...
${third.length ? `          bash ../clone-plugins.sh "$PWD/source"` : '          echo "无第三方源码插件"'}
      ...
      - name: 配置
        run: |
          ...
          make defconfig
          wc -l tmp/.packageinfo

      - name: 下载
        run: cd source && make download -j$(nproc) V=s

      - name: 编译
        run: cd source && make -j$(nproc) V=s
...
function estimateSRC(spec) {
  ...
  const diskGB = 40 + (third.length ? third.length * 0.35 : 0);
  return {
    engine: 'src',
    seconds,
