...
class Gh {
  ...
  /**
   * 下载单个 artifact 的 zip（接口返回 302，需自己跟 Location）。
   *
   * firmware 这类产物动辄几百 MB（实测 386MB），整包流式下载只要网络一抖就前功尽弃，
   * 报错还永远是干巴巴的 terminated——看不出进度、也续不上。所以这里按 Range 分片：
   * 单片失败只重来这一片（默认 8MB／片），中断后再进来直接用已落盘的部分继续。
   */
  async downloadArtifact(artifactId, destFile, opt = {}) {
    const { chunk = 8 * 1024 * 1024, retries = 5, onProgress } = opt;
    ...
    try {
      while (written < total) {
        ...
        const buf = await this.fetchRange(finalUrl, written, end, retries);
  ...
  async fetchRange(url, start, end, retries) {
    let last = null;
    for (let i = 0; i <= retries; i++) {
      const ctrl = new AbortController();
      const timer = setTimeout(() => ctrl.abort(), 120000); // 单片 8MB，2 分钟还没下完就是有问题
      try {
        const res = await fetch(url, {
