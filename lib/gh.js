...
class Gh {
  ...
  async defaultBranch() {
    ...
    const info = await this.repoExists();
    if (info && info.default_branch) this._defaultBranch = info.default_branch;
   ...
   * 默认分支上**真的**有这个文件吗？
   *
   * 千万别信 `/actions/workflows` 列表：它会残留"幽灵注册"——文件早已从默认分支删掉，
   * 列表里却仍显示 `state=active`（实测：删掉 .github/ 之后 ib-build.yml 与 loop-probe.yml
   * 都还在列表里挂着）。信它就会得到 `registeredOnDefault: true`，
   * 然后 dispatch 被 GitHub 用 422 "Workflow does not have 'workflow_dispatch' trigger" 打回来。
   * 所以这里一律以 contents 接口（默认分支上的真实文件）为准。
   */
  async fileOnDefault(path) {
    const br = await this.defaultBranch();
    const r = await this.req('GET', `/repos/${this.repo}/contents/${encodeURI(path)}?ref=${encodeURIComponent(br)}`);
    return r.ok ? r.json : null;
  }
  ...
  async isRegisteredOnDefault(workflowFile) {
    return await this.fileOnDefault(`.github/workflows/${workflowFile}`);
  }

  /**
   * 把工作流文件补到默认分支（幂等：内容一致时 contents 接口不产生新提交）。
   * 只推这一个文件，不把 build.sh / spec.json 之类的构建包混进默认分支。
   */
  async putFileOnDefault({ path, content }, message) {
    const br = await this.defaultBranch();
    const cur = await this.fileOnDefault(path);
    const body = {
      message,
      content: Buffer.from(content, 'utf8').toString('base64'),
      branch: br,
    };
    // 已存在就必须带 sha，否则 422；内容没变时 GitHub 会直接返回 200 且不产生提交
    if (cur && cur.sha) body.sha = cur.sha;
    const r = await this.req('PUT', `/repos/${this.repo}/contents/${encodeURI(path)}`, body);
    if (!r.ok) throw new Error(`补推 ${path} 到 ${br} 失败：${r.json && r.json.message ? r.json.message : 'HTTP ' + r.status}`);
    return { branch: br, path, committed: !(r.json && r.json.commit && cur && cur.sha === r.json.content.sha) };
  }
  ...
  async dispatch(workflowFile, branch, inputs = {}) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      // 404 = 尚未注册；422 = 列表里还有幽灵记录、但默认分支上其实没这个文件
      //（刚补推上去时也需要几秒才被注册），两者都值得等一下重试。
